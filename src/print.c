...
/*   Updated: 2025/04/13 20:48:54 by ygille           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (content)
	{
		if (!is_unwanted(content->name, param))
		{
			if (!first)
				...
				first = false;
			ft_printf("%c%s %d %s %s %d %s", content->mode, content->perms, content->links, content->owner, content->group, content->size, content->name);
		}
		content = content->next;
	}
}
